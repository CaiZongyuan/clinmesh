...
Surface Agent controller 把当前页面注册投影成 DSH Session-scoped browser Tools。Client adapter 订阅 DSH 唯一的 current-session store，并把当前 Session ID 作为 Web runtime 输入；切换 Session 会立即撤下旧 Page Context 与 review。Capability availability 由 DSH bridge 拥有，publisher 必须先发布 registration 才能触发探测，因此 Page Context 和 registration 不以 `unavailable`、`idle`、`connecting` 或 `active` 为前置；只有 detached review 要求 lease 保持 `active`，离开时立即取消。Page Context、Tool catalog、proposal 和 review contract 位于 `packages/contracts` 与 Server；DSH adapter 不导入 Repository、Workflow 状态机或 Hidden Fact。默认布局是 `workspace`，空间不足时由 Surface runtime 退化到 `full-frame`，隐藏时 keep-alive 保留客户端草稿。
