...
- 默认 `workspace` 左右分屏，窄空间按比例收缩并由应用内部响应式适配；全屏仅由用户手动选择，应用顶部提供返回分屏入口。切换保留原生侧栏状态、分栏偏好与应用草稿，详见[手动全屏与容器响应式](../.agents/notes/implemented/bug-fix/2026-09-11-surface-responsive-layout.md)。
