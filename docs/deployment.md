...
重新启动 DSH Web 后，左侧栏顶端显示 ClinMesh Logo 和名称；应用尚未打开或已经关闭时仍保留该 Profile 品牌。新会话中央同样显示 ClinMesh Logo，中文标题为“医疗智能体平台”，英文为“Medical AI Agent Platform”。禁用或卸载 ClinMesh 插件后恢复宿主的品牌显示。从侧栏的 React applications launcher 打开 ClinMesh；Profile 只有一个 Surface 时直接打开，多个 Surface 时显示选择器。默认使用 `workspace` 左右分屏并保留原生会话；应用左侧导航栏底部的“全屏 ClinMesh”与“返回 DSH 分屏”按钮可往返切换，无需刷新。窗口缩小或侧栏开关不自动全屏，应用内部按容器宽度适配。页面导航使用 Memory Router，不修改 DSH document pathname。当前模式只信任安装到同一 Web Profile 的插件，并只允许合成数据。
