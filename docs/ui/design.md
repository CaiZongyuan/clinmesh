...
| 检验 | 检验项目与适应证、草稿、开立、取消、生成重试、报告、已阅状态和报告修订 |
| 诊断 | 受控诊断目录、主次诊断、草稿和正式确认 |
| 处方 | 用药结论、按临床产品分组的真实包装选择、剂量、频次、疗程、数量、草稿、正式开具和撤回 |
...
未提交的 Clinical Document 输入以 `caseId` 隔离保存在当前前端会话状态中；切换页签或患者不会丢失输入，也不会将内容带入另一病例，刷新或关闭页面会丢弃未提交内容。诊断、检验和处方草稿通过各自的显式保存动作持久化。界面不显示草稿版本、FHIR 版本或乐观锁版本，服务端继续执行 expected version、幂等、审计和冲突检查。
