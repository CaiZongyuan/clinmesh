...
未提交的 Clinical Document 输入以 `caseId` 隔离保存在当前前端会话状态中；切换页签或患者不会丢失输入，也不会将内容带入另一病例，刷新或关闭页面会丢弃未提交内容。诊断、检验和处方的有效修改在短暂静止后自动保存，界面只显示等待、保存中、已保存或保存失败，不提供普通“保存草稿”按钮。删除整个草稿、确认诊断、开立申请和正式开具仍是明确命令。界面不显示草稿版本、FHIR 版本或乐观锁版本，服务端继续执行 expected version、幂等、审计和冲突检查。
