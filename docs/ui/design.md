...
| 检验 | 当前病例可生成结果的检验项目、真实存在的多选适应证、自动保存草稿、开立、取消、生成重试、报告、已阅状态和报告修订 |
...
患者横幅常驻“完诊”动作。存在阻断项时，Dialog 显示服务端完诊清单并可跳转到对应页签；满足条件后才允许确认完诊。已完诊病例默认只读，服务端允许的纠错入口先激活目标页签，再将焦点和滚动位置移到目标区域。
