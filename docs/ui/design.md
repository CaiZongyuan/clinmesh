...
| 检验 | 可搜索的检验目录、当前病例生成能力、真实存在的多选适应证、自动保存草稿、开立、取消、生成重试、报告、已阅状态和报告修订 |
...
患者横幅常驻“完诊”动作。存在阻断项时，Dialog 显示服务端完诊清单并可跳转到对应页签；满足条件后才允许确认完诊。已完诊病例默认只读，服务端允许的纠错入口先激活目标页签，再将焦点和滚动位置移到目标区域。检验选择器保留搜索结果的可发现性，同一 LOINC system 与 code 只显示一个项目；当前病例不可生成的项目显示为禁用并标明原因。
