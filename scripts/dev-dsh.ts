...
export async function ensureDshRuntimeReady(
  ...
  const currentSurfaceCommit = (await dependencies.runCommand('git', ['rev-parse', 'HEAD'], checkout)).trim()
  ...
  const currentAgUiCommit = (await dependencies.runCommand('git', ['rev-parse', 'HEAD'], sandbox.agUiDir)).trim()
...
function createRealRunCommand(): DshEnsureDependencies['runCommand'] {
  return (command, args, cwd, environment) => new Promise((resolveCommand, reject) => {
    ...
    child.once('close', (code) => {
      if (code === 0) resolveCommand(Buffer.concat(chunks).toString('utf8'))
      else reject(new Error(`${command} ${args.join(' ')} 退出码 ${code ?? 'signal'}`))
    })
  })
}
