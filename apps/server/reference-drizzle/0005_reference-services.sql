ALTER TABLE reference_release
  ADD COLUMN service_count INTEGER NOT NULL DEFAULT 0
  CHECK (service_count >= 0);

ALTER TABLE reference_release
  ADD COLUMN value_set_entry_count INTEGER NOT NULL DEFAULT 0
  CHECK (value_set_entry_count >= 0);

CREATE TABLE reference_source_manifest_v5 (
  ...
  artifact_format TEXT NOT NULL CHECK (
    artifact_format IN (
      'clinmesh-reference-v1', 'loinc-csv', 'nhc-medical-service-csv',
      'nhsa-diagnosis-csv', 'nhsa-medication-product-csv', 'ucum-xml',
      'wst-value-set-csv'
    )
  ),
...
INSERT INTO reference_source_manifest_v5 (
...
CREATE TABLE reference_concept_v5 (
  release_id TEXT NOT NULL,
  ...
  FOREIGN KEY (release_id, source_id)
    REFERENCES reference_source_manifest_v5 (release_id, source_id) ON DELETE RESTRICT
) STRICT;

INSERT INTO reference_concept_v5 (
...
CREATE TABLE reference_medication_product_v5 (
  release_id TEXT NOT NULL,
...
INSERT INTO reference_medication_product_v5 (
...
ALTER TABLE reference_source_manifest_v5 RENAME TO reference_source_manifest;
ALTER TABLE reference_concept_v5 RENAME TO reference_concept;
ALTER TABLE reference_medication_product_v5 RENAME TO reference_medication_product;
...
CREATE TABLE reference_medical_service (
  release_id TEXT NOT NULL,
...
CREATE TABLE reference_value_set_entry (
  release_id TEXT NOT NULL,
