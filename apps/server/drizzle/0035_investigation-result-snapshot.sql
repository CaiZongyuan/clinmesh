CREATE TABLE investigation_result_snapshot (
  ...
  UNIQUE (workspace_id, case_id, catalog_item_id),
  FOREIGN KEY (workspace_id, case_id)
    REFERENCES synthetic_case_instance (workspace_id, case_id) ON DELETE RESTRICT,
  CHECK (
    (source = 'synthea-exact' AND model_id IS NULL AND prompt_version IS NULL AND prompt_hash IS NULL)
    OR (source = 'investigation-agent' AND model_id IS NOT NULL AND prompt_version IS NOT NULL AND prompt_hash IS NOT NULL)
  )
) STRICT;

ALTER TABLE laboratory_report_acknowledgement
  RENAME TO laboratory_report_acknowledgement_legacy_v2;
ALTER TABLE laboratory_report_revision
  RENAME TO laboratory_report_revision_legacy_v2;
ALTER TABLE laboratory_request RENAME TO laboratory_request_legacy_v2;
...
INSERT INTO laboratory_request (
  ...
  status, version, authored_by, authored_at, accepted_at,
  started_at, reported_at, acknowledged_at, cancelled_at
)
SELECT
  workspace_id, epoch, request_id, case_id, catalog_item_id, reference_json,
  NULL, indication_code, service_request_id, execution_task_id,
  diagnostic_report_id, NULL, NULL, status, version, authored_by, authored_at, accepted_at,
  started_at, reported_at, acknowledged_at, cancelled_at
FROM laboratory_request_legacy_v2;
...
SELECT * FROM laboratory_report_acknowledgement_legacy_v2;
...
SELECT * FROM laboratory_report_revision_legacy_v2;

DROP TABLE laboratory_report_acknowledgement_legacy_v2;
DROP TABLE laboratory_report_revision_legacy_v2;
DROP TABLE laboratory_request_legacy_v2;
