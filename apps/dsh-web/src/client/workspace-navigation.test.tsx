...
it('places authorized routes above workspaces, keeps settings in the footer, and cleans up on unload', async () => {
  ...
  try {
    ...
    await act(() => root.render(<WorkspaceNavigation navigation={navigation} wide open={open} locale="en-US" />))
    expect(trigger.getAttribute('aria-label')).toBe('Hospital workspace')
    await act(() => root.render(<WorkspaceNavigation navigation={navigation} wide open={open} locale="zh-CN" />))
    open.mockClear()
    navigate.mockClear()
    await act(() => trigger.click())
