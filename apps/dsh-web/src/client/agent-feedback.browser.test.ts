...
it.each(['18', '19'])('preserves editing and human review with action feedback on React %s', async version => {
  ...
  const actual = z.object({ focused: z.boolean(), highlighted: z.boolean(), runningAnimation: z.string(), faded: z.boolean(), waiting: z.boolean(), staticWaiting: z.boolean(), committed: z.boolean(), approved: z.boolean() }).parse(response)
  expect(actual).toEqual({ focused: true, highlighted: true, runningAnimation: 'clinmesh-agent-flow', faded: true, waiting: true, staticWaiting: true, committed: true, approved: true })
}, 30_000)
