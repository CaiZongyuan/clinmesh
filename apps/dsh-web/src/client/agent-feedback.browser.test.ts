...
it.each(['18', '19'])('preserves editing and human review with action feedback on React %s', async version => {
  ...
  const actual = z.object({ focused: z.boolean(), highlighted: z.boolean(), aligned: z.boolean(), runningAnimation: z.string(), faded: z.boolean(), waiting: z.boolean(), staticWaiting: z.boolean(), committed: z.boolean(), approved: z.boolean(),
    consultationRegion: z.boolean(), newDoctorBubble: z.boolean(), newPatientBubble: z.boolean(), oldMessageUnchanged: z.boolean(),
  }).parse(response)
  expect(actual).toEqual({ focused: true, highlighted: true, aligned: true, runningAnimation: 'clinmesh-agent-flow', faded: true, waiting: true, staticWaiting: true, committed: true, approved: true,
    consultationRegion: true, newDoctorBubble: true, newPatientBubble: true, oldMessageUnchanged: true,
  })
}, 30_000)
