...
Client adapter 通过官方 `settings.general.item` 注册独立的 ClinMesh 字号控件，与 Surface 共享一个可订阅的偏好源。Web runtime 注入生效字号，WebApp 在渲染时派生显示偏好，不重建应用或把注入值写回独立 Web 偏好。语言继续遵循[宿主语言归属](2026-09-16-dsh-host-locale-ownership.md)。
