...
未知 HTTP 异常与后台 dispatch failure 通过同一 runtime reporter 写入结构化 stderr。reporter 只接受受控 scope 和元数据，记录 correlation ID、HTTP method/path、错误类型，以及去掉首行 message 并筛选后的有限 stack frame；已知认证、授权、validation 和业务 conflict 不重复记为未知运行异常。
...
**在错误响应和日志中保留原始 message 或完整 stack。** 诊断信息更多，但异常可能包含患者内容、查询值、凭证或外部 provider 文本；响应使用固定诊断，日志仅保留受控字段与筛选后的 frame。
