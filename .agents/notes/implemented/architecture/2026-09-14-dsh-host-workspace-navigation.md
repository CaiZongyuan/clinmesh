...
主题选择与导航注册中的主题字段由[DSH 统一管理 Surface 主题](2026-09-15-dsh-host-theme-ownership.md)取代；下述主题操作说明保留原决策依据，其余导航与生命周期合同继续有效。
...
ClinMesh 业务请求返回 401 时，公共 API client 通知当前应用，将 TanStack Query 拥有的会话查询转为认证错误；工作台卸载并撤销宿主导航，登录成功后重新查询会话并恢复入口。网络失败、403 和服务器错误保留当前会话。登录失败和会话查询由各自认证入口处理，DSH execution proof 使用独立宿主会话，不触发 ClinMesh 退出。请求绑定发起时的通知 owner 和登录代次；应用卸载、登录或退出成功后，旧请求的迟到响应不能影响后续会话。
...
**各业务页面分别处理会话过期。** 容易遗漏直接 API 调用和 Agent action，也会复制退出规则；公共请求层只识别认证失败，会话状态与导航清理仍由应用拥有。
