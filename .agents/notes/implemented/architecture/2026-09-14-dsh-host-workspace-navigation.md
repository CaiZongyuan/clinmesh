...
ClinMesh 业务请求返回 401 时，公共 API client 通知当前应用，将 TanStack Query 拥有的会话查询转为认证错误；工作台卸载并撤销宿主导航，登录成功后重新查询会话并恢复入口。网络失败、403 和服务器错误保留当前会话。登录失败和会话查询由各自认证入口处理，DSH execution proof 使用独立宿主会话，不触发 ClinMesh 退出。请求绑定发起时的通知 owner，应用卸载后到达的响应不能通知后续应用实例。
