...
description: Record browser or Web UI interaction demos as optimized GIFs using the available browser, state-based frame capture, and deterministic encoding; optionally publish the artifact to a pull request assets branch.
---
...
One storyboard comes from one isolated run. Publication revalidates both the artifact and the demonstrated pull-request commit.

## GUI evidence

Use a demonstration GIF when the user requests one or when a pull request needs compact visual evidence of a multi-state workflow.
...
1. Require a clean worktree, record its exact commit with `git rev-parse HEAD`, then build that recorded tree with `pnpm build`. A GIF recorded against another commit's build misattributes the evidence.
2. Boot one server per port from that tree with a fresh synthetic workspace/epoch and isolated browser context. If the browser workflow cannot create one, clear that origin's cookies and site storage before navigation so persisted client state cannot affect the evidence. Source configuration through the application's normal path; never echo a key.
